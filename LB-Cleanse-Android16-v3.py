...
class OCILoadBalancerGhostHunter:
    ...
    def scan_load_balancers(self, compartment_ids: Optional[List[str]] = None):
        ...
        # IMPORTANT: compute suspicious list BEFORE summary (so summary can show details)
    ...
    def generate_html_report(self, html_path: str):
        ...
        try:
            ...
            compartment_list = ", ".join(sorted(set([lb['Compartment'] for lb in self.all_load_balancers]))) or "N/A"
            ...
            healthy_count = total_scanned - total_ghosts

            html_content = self._generate_html_content(
                report_date, compartment_list, total_scanned, total_ghosts, definite_ghosts, healthy_count
            )
    ...
    def _generate_html_content(self, report_date: str, compartment_list: str,
                              total_scanned: int, total_ghosts: int, definite_ghosts: int, healthy_count: int) -> str:
        """Generate the HTML content for the report"""

        # Healthy section (ALWAYS RENDERED)
        healthy_section = f"""
            <div class="section">
                <h2>✅ Healthy Load Balancers</h2>
                <div class="metadata" style="border-left-color:#27ae60;">
                    <p><strong>{healthy_count}</strong> load balancers appear to be properly configured and in use.</p>
                </div>
            </div>
        """

        # Suspicious table rows
        table_rows = ""
        if self.suspicious_load_balancers:
            for ghost in sorted(self.suspicious_load_balancers, key=lambda x: x['GhostScore'], reverse=True):
...
{no_ghosts_section}
        ...
        <div class="footer">
            <p>Generated by OCI CAMehameha Repository</p>
